 ...
 */
function buildGraph(parseResults, unchangedFiles, previousGraph, root, fileHashes, version, startTime, incremental) {
    const allNodes = [];
    const allEdges = [];
    const files = {};
    const symbols = {};
    ...
    for (const [symbolName, symbolData] of Object.entries(symbols)) {
        if (!symbolData.calls || !Array.isArray(symbolData.calls))
            continue;
        for (const callTarget of symbolData.calls) {
            if (!callTarget || typeof callTarget !== "string")
                continue;
    ...
    for (const [filePath, fileData] of Object.entries(files)) {
        ...
        for (const imp of fileData.imports) {
            ...
            for (const candidate of candidates) {
                if (files[candidate]) {
                    if (!dependsOn.includes(candidate))
    ...
    const totalInboundEdges = Object.values(relationships).reduce((sum, r) => sum + r.dependedOnBy.length, 0);
    const maxInbound = Math.max(1, ...Object.values(relationships).map((r) => r.dependedOnBy.length));
    for (const [filePath, fileData] of Object.entries(files)) {
    ...
    const maxCalledBy = Math.max(1, ...Object.values(symbols).map((s) => s.calledBy.length));
